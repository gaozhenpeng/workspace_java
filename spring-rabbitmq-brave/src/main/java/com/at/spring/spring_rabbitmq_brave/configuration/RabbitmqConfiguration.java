...
import com.at.spring.spring_rabbitmq_brave.listener.DirectMessageListener;
import com.at.spring.spring_rabbitmq_brave.listener.HeaderMessageListener;
import com.at.spring.spring_rabbitmq_brave.postprocessor.AfterReceivePostProcessor;
import com.at.spring.spring_rabbitmq_brave.postprocessor.BeforePublishPostProcessor;
...
//		return BindingBuilder.bind(myDirectQueueForListener()).to(directExchange()).withQueueName(); // routingkey: mydirectqueueforlistener
		return BindingBuilder.bind(myDirectQueueForListener()).to(directExchange()).with(myDirectQueueForListener().getName()+ "key"); // routingkey: mydirectqueueforlistenerkey
	}
...
//		return BindingBuilder.bind(myHeadersQueue()).to(headersExchange()).whereAny(headers).match();
	...
	public SimpleMessageListenerContainer messageListenerContainerForDirect() {
		logger.debug("SimpleMessageListenerContainer for DirectQueue");
	...
	public SimpleMessageListenerContainer messageListenerContainerForHeader() {
		logger.debug("SimpleMessageListenerContainer for HeaderQueue");
		SimpleMessageListenerContainer container = new SimpleMessageListenerContainer();
		container.setConnectionFactory(connectionFactory());
		container.setAfterReceivePostProcessors(afterReceivePostProcessor);
